...
- **Formal CI (`agda` / `coq` / `lean` jobs)** — register **`Agda/umst-formal.agda-lib`** plus a tiny **`standard-library`** stub `.agda-lib` pointing at **`/usr/share/agda-stdlib`** (Ubuntu package omits upstream’s library file); run **`coq`** via `docker run coqorg/coq:8.20` with **`chmod a+rwX Coq`** before bind-mount (default `coq` user; avoid `--user root` which breaks opam); post **`chown`** for cleanliness; **`leanprover/lean-action@v1`** with `lake build UMST` (no `--wfail` on CI to avoid Mathlib replay flaking on upstream linter noise).
