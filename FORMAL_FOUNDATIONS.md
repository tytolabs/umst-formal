...
| `^axiom ` in `Lean/*.lean` (excluding `.lake`) | **1** physics — `LandauerLaw.physicalSecondLaw`; plus **33** tier-tagged axioms in `Crypto/` + `Behavior/SDFCanonical.lean` (`check_lean_axioms.py`) |
...
| Axiom policy | **`physicalSecondLaw`** + tier-tagged crypto/behavior axioms in allowed files; CI: `scripts/check_lean_axioms.py` |
